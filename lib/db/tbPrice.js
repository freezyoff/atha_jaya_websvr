const { dbAll, dbQuotes, dbRun, isDbError } = require("../db");
...
class TbPrice {
   ...
   static delete(data, callback){
      ...
      let sqlStmt = `DELETE FROM ${this.tableName} WHERE ?;`;
      sqlStmt = sqlStmt.replace("?", whereStmt.join(" AND "));
      dbRun(sqlStmt, (err, rows)=>{
         if (isDbError(err)) callback(err);
         else callback(rows);
      });
   }

   static deleteSync(data){
      return new Promise(resolve=>{
         TbPrice.delete(data, result=>{
            resolve(result);
         })
      })
   }
